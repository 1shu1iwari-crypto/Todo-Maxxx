...
export function initFaultyTerminal(container) {
  const options = {
    scale: 1,
    gridMul: [2, 1],
    digitSize: 1.5,
    ...
    dither: 0,
    curvature: 0.2,
    tint: '#00ff41', // matrix green, you can change this!
    mouseReact: true,
    mouseStrength: 0.2,
  ...
  const ditherValue = options.dither;
  ...
  return () => {
    ...
    if (options.mouseReact) container.removeEventListener('mousemove', handleMouseMove);
...
if (bgContainer) {
    initFaultyTerminal(bgContainer);
}
